...
function openScanPayQrModal(method) {
  ...
  if (qr && config.qr) {
    const versioned = `${config.qr}${config.qr.includes('?') ? '&' : '?'}v=20260712b`;
    qr.onerror = null;
    delete qr.dataset.triedFallback;
    qr.onerror = () => {
      if (qr.dataset.triedFallback) return;
      qr.dataset.triedFallback = '1';
      if (config.qr.endsWith('.png')) {
        qr.src = config.qr.replace(/\.png$/, '.jpg') + '?v=20260712b';
      }
    };
    qr.src = versioned;
